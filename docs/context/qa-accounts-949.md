...
QA test accounts created on host **192.168.1.96:8989** (all password `Qa@Passw0rd123`,
created via mock OTP `123456` which this host honors; old host 192.168.0.66 did not).
Creation flow: `/v2/common/sendOTP` → `/v2/common/validateOTP` → registration.

| Usertype | kpostID | Mobile | Register endpoint | maximumMembersCount |
|---|---|---|---|---|
| PERSONAL (BROKEN) | meera949@kpostindia.com | 9000000949 | `/v2/signupLogin/signup` | — |
| PERSONAL (PRIMARY) | meera951@kpostindia.com | 9000000955 | `/v2/signupLogin/signup` | — |
...
| PERSONAL (victim) | meera950@kpostindia.com | 9000000953 | `/v2/signupLogin/signup` | — |
...
IDOR/messaging counterparty — fresh, owns a profile; seed it with mail/messages later for richer
message/mail-IDOR); `QA_BUSINESS_RECEIVER_KPOST_ID` = **meera@m949m.kpost.in** (our verified BUSINESS_M).

**Tier member caps are exact per-tier values the backend validates** — S=10, M=250, L=2000.
Any other count returns HTTP 400 `"Invalid maximumMembersCount for userType"`. All three
business tiers use the SAME `adminRegistration` endpoint (not the mediumAndLarge signup — that
...
**meera949 BROKE 2026-09-08 (~after the 12:32 run) — ROOT CAUSE CONFIRMED DB-WISE 2026-09-09: its
...
user_type=PERSONAL as the working accounts — fully healthy. The `password` column is reversible AES,
**deterministic** (same plaintext → byte-identical ciphertext): meera950/meera951/m949s/m949m/m949l ALL
share ciphertext `rcGAFVCq1nz5WDfkyzogNg==` (= `Qa@Passw0rd123`), but meera949 alone holds a different
value `AtpYv2vxusSdprsxe3N9iA==` → its password is no longer Qa@Passw0rd123. Cause: meera949 was the
shared QA_KPOST_ID (created 08:15:07, earliest of the batch) when a `changePassword` test fired at the
shared session during the 12:32 run — BEFORE the safety fix routed changePassword/deactivate to
...
then Qa@Passw0rd123 works again. **APPLIED & VERIFIED 2026-09-09:** user ran the UPDATE; meera949 logs in again. (The two `password` vs `kmail_password` columns: KPost login reads the
AES `password`; `kmail_password` is a separate bcrypt hash.) Replaced by **meera951@kpostindia.com** (9000000955) as the
SHARED primary in BOTH `.env` (QA_KPOST_ID) and `kmail/.env` (QA_KPOST_ID) — the user requires the SAME
user in both modules. QA_ADMIN=meera@m949s, victim=meera950 unchanged. Cleared stale `.auth/session.json`
in both suites (KMail cache still held the old meeravelu23 → caused KMail to run WITHOUT a session, 491
...
auth (`:8989`) with `401 "Invalid Data"` on protected routes — on BOTH hosts (192.168.0.66 and
...
(verified 2026-09-09) — EVERY account opens KMail.** Clean unique-device probes (login @ :8989 → token →
...
its OWN device and every one returns 200. **So there is NO KMail block and NO provisioning gap** — the
whole "KMail blocked on backend" saga is resolved (developers fixed the 9081↔8989 JWT trust). **KMail
...
sharing one session via .auth/session.json (verified by tokenOpensKmail before reuse) — same-user
...
lands, re-verify in seconds:** login any account at 8989 → present token to `GET :9081/v2/kmailData/getKloudUsedData`;
non-401 = fixed → run `npm run test:kmail` (still BUGZILLA_DRY_RUN=true). KMail suite/accounts/payloads all ready.

`admin/.env` was NOT changed — that bench is a different backend
