...
/*
 * Offline unit coverage for `getNextAlias` â€” no Bugzilla instance, no network. `globalThis.fetch`
 ...
 * runtime, so the only defence is asking for the full set up front (`limit=0`) â€” and the only
...
/** `KPA-001 â€¦ KPA-<count>`, ascending, so the highest number is the last one served. */
function ascendingAliases(count: number): string[] {
  return Array.from({ length: count }, (_, index) => `KPA-${String(index + 1).padStart(3, '0')}`);
}
...
test.describe('getNextAlias - pagination', () => {
  ...
  test('computes the max across a product larger than one page', async () => {
    ...
    const alias = await getNextAlias(config, 'KPA');
    ...
    expect(
      alias,
      'next alias must follow the highest alias in the whole product (KPA-250), not the highest on page one'
    ).toBe('KPA-251');
  });
  ...
  test('does not reissue an alias that exists beyond the first page', async () => {
    ...
    // KPA-021 is what a first-page-only max yields, and KPA-021 is already taken - Bugzilla
    // would reject the create, and the caller would misreport it as "filing without alias".
    expect(
      alias,
      'a first-page-only max would mint KPA-021, an alias already held by an existing bug'
    ).not.toBe('KPA-021');
  });
  ...
  test('finds the highest alias when it sits on the last page behind lower ones', async () => {
    ...
    const aliases = [...ascendingAliases(40), 'KPA-900', ...ascendingAliases(5)];
    ...
    expect(
      alias,
      'the maximum must win regardless of where it falls in the result order - KPA-900 sits well past the page boundary'
    ).toBe('KPA-901');
  });
