 ...
 * missing â€” the log said "create response missing id" and the real message (a duplicate alias)
 * was discarded. And because an orphaned `KPA-001` row survived a rolled-back create while the
...
const DUPLICATE_ALIAS_BODY = {
  ...
  code: 100500,
  message:
    "DBD::MariaDB::st execute failed: Duplicate entry 'KPA-001' for key 'bugs_aliases_alias_idx' [for Statement \"INSERT INTO bugs_aliases\"] at Bugzilla/Bug.pm line 835.\n\tBugzilla::Bug::create called at line 744",
};
 ...
 * blind to them â€” the live instance's exact state.
 */
...
test.describe('Bugzilla create error handling', () => {
  ...
  test('an HTTP 200 carrying error:true is reported as a failure, not a missing id', async () => {
    stubBugzilla(['KPA-001']);

    const result = await createBug(config, { ...fields, alias: ['KPA-001'] });
  ...
  test('a taken alias is stepped over instead of failing the ticket', async () => {
    const server = stubBugzilla(['KPA-001']);

    const result = await createBugWithAlias(config, fields, 'KPA');

    expect('error' in result, 'an unavailable alias must not cost the defect its ticket').toBe(false);
    if (!('error' in result)) {
      expect(result.alias, 'KPA-001 was taken, so the next free number must be used').toBe('KPA-002');
    }
    expect(server.createdAliases, 'exactly one bug should exist, under the free alias').toEqual(['KPA-002']);
  });
  ...
  test('consecutive defects keep advancing when the search cannot see prior aliases', async () => {
    ...
    const first = await createBugWithAlias(config, fields, 'KPA');
    const second = await createBugWithAlias(config, fields, 'KPA');
    const third = await createBugWithAlias(config, fields, 'KPA');
    ...
    expect(
      ...
      'a blind search must not make every defect ask for KPA-001 - that is the loop that filed nothing'
    ).toEqual(['KPA-001', 'KPA-002', 'KPA-003']);
  });

  test('a module with no matching component is filed under the fallback', () => {
    const valid = new Set(['Knews', 'kpost-webservice-application']);

    expect(
      resolveComponent('Unclassified', valid, 'kpost-webservice-application'),
    ...
    ).toBe('kpost-webservice-application');
  });
  ...
  test('a module that does have a component is left alone', () => {
    ...
    expect(
      resolveComponent('Knews', valid, 'kpost-webservice-application'),
  ...
  test('an unreadable component list changes nothing', () => {
    expect(
      resolveComponent('Knews', new Set(), 'kpost-webservice-application'),
