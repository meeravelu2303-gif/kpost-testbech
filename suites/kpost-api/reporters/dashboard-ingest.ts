...
// `require.main === module` path below, mirroring dashboard-bugzilla.ts. Without it a manual
// push reports "DASHBOARD_INGEST_URL not configured" on a bench that has it configured.
...
export async function pushToDashboard(
  ...
  /*
   * Run-validity gate. A zero-test or collapsed run must never become a dashboard point:
   * it renders as a completed run with 0 defects, which on a trend line reads as the API
   * having *improved*. This gate is why `--list` and a failed-to-load suite no longer
   * publish — both previously did, as runIds 5 and 6.
   */
...
/*
 * Standalone entry, mirroring `dispatch-bugs-to-tracker.ts`.
 *
 * The reporter publishes automatically at the end of a run, but the dashboard's
 * ingest is an upsert keyed on the defect fingerprint, so re-pushing the same
 * `BUG_REPORT.json` is safe and idempotent — it refreshes an existing run's
 * defects rather than duplicating them. That makes a manual push the repair path
 * when a run's publish was interrupted, or when the dashboard's copy has been
 * cleared and needs the current report back.
 *
 * Exit code follows the outcome here, unlike the reporter, which must never
 * change a test run's verdict: run by hand, a silent failure to publish is the
 * one thing worth failing on.
 */
