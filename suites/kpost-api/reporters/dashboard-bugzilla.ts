...
export interface BugzillaConfig {
   ...
   *
   * Off by default, because the instance at 192.168.0.50 already assigns `KPA-<bugid>` on
   ...
   * 'Unclassified' in the 'KPost API' product."* Those tickets were silently lost while the QA
  ...
  maxFile: number;
  /**
   * File "Assertion Failure" records to Bugzilla as well.
   *
   * Off by default. Those records are TESTS that broke - a bare expect(), a timeout, a
   * fixture fault - not APIs that broke. They carry no curated severity or category, and
   * they are frequently many symptoms of one environment problem: on the run that
   * motivated the filter they were 296 of 387 records.
   *
   * Turn it on (BUGZILLA_FILE_ASSERTION_FAILURES=true) when the tracker is deliberately
   * being rebuilt from zero and the intent is a complete census of one run. Be aware that
   * the extra tickets are not defects a backend developer can fix, so a fix-rate measured
   * across them understates the real one.
   */
  fileAssertionFailures: boolean;
}
...
export function readBugzillaConfig(): { config: BugzillaConfig } | { skip: string } {
  ...
  return {
    config: {
      ...
      fallbackComponent: process.env.BUGZILLA_FALLBACK_COMPONENT || 'kpost-webservice-application',
      maxFile: Math.max(0, Number.parseInt(process.env.BUGZILLA_MAX_FILE ?? '', 10) || 0),
      fileAssertionFailures: process.env.BUGZILLA_FILE_ASSERTION_FAILURES === 'true',
    },
  };
}
...
export const PRIORITY_MAP: Record<Defect['priority'], string> = {
  P0: 'Highest',
  P1: 'High',
  P2: 'Normal',
  P3: 'Low',
...
/**
 * Bugzilla's `short_desc` column is 255 characters and the API refuses anything longer with
 * `HTTP 400 - The text you entered in the Summary field is too long`.
 *
 * A defect title is written for a human reading BUG_REPORT.md, where length costs nothing, so
 * some carry a response body or a full sentence of consequence. Three such defects were
 * refused on 2026-08-28 — and a refusal here is the expensive kind of failure: the run reports
 * "not filed" for one line among a hundred successes, so a real defect quietly never reaches
 * the tracker.
 *
 * The `[<id>]` tag is preserved intact because `findExistingOpenBug` searches on it; only the
 * title is trimmed, and the untruncated text is still in the description body below.
 */
const BUGZILLA_SUMMARY_MAX = 255;

export function fitSummary(tag: string, title: string): string {
  const full = `${tag} ${title}`;
  if (full.length <= BUGZILLA_SUMMARY_MAX) return full;
  // -1 for the space after the tag, -1 for the ellipsis that marks the trim.
  const room = BUGZILLA_SUMMARY_MAX - tag.length - 2;
...
/**
 * Resolutions that mean "a human looked at this and it is not a defect". A ticket closed
 * with one of these must never be re-filed under a new number: the suite re-observes the
 * same behaviour on every run, so re-filing turns one settled judgement into a nightly
 * duplicate.
 */
...
export async function findExistingOpenBug(config: BugzillaConfig, defect: Defect): Promise<FindResult> {
  const tag = `[${defect.id}]`;
  const query = `/bug?summary=${encodeURIComponent(defect.id)}&product=${encodeURIComponent(config.product)}&include_fields=id,is_open,summary,resolution`;
  const result = await bugzillaCall(config, 'GET', query);
  if (!result.ok) return { error: describeFailure(result) };

  const bugs =
    (result.json as
      | { bugs?: Array<{ id: number; is_open?: boolean; summary?: string; resolution?: string }> }
      | undefined)?.bugs ?? [];
  const tagged = bugs.filter((bug) => typeof bug.summary === 'string' && bug.summary.includes(tag));

  const open = tagged.find((bug) => bug.is_open !== false);
  if (open) return { id: open.id };

  // Closed as not-a-defect by a human. A wrong judgement is corrected by reopening that
  // ticket, never by minting a second one for the same finding.
  const judged = tagged.find((bug) =>
    JUDGED_NOT_A_DEFECT.has(String(bug.resolution ?? '').toUpperCase()));
  if (judged) return { judged: judged.id, resolution: String(judged.resolution) };

  // Closed, but not by a human calling it a non-defect - so it was closed FIXED (or
  // resolved some other way) and the fault is back. Reopening keeps one ticket per fault;
  // filing a new one is how an original and its re-file end up side by side.
  const stale = tagged.find((bug) => bug.resolution);
...
/**
 * Puts a resolved ticket back into play because its defect was observed again.
 *
 * Status is set to CONFIRMED rather than the installation-specific REOPENED, which not
 * every Bugzilla workflow defines; clearing the resolution is what actually reopens it.
 */
export async function reopenBug(
  ...
  const result = await bugzillaCall(config, 'PUT', `/bug/${bugId}`, {
    status: 'CONFIRMED',
    resolution: '',
...
export interface DefectOutcome {
  defectId: string;
  bugzillaId?: number;
  action: 'created' | 'commented' | 'reopened' | 'failed' | 'dry-run' | 'skipped-judged';
  error?: string;
}
...
export async function fileBugzillaDefects(
   ...
   */
  const FILE_TO_BUGZILLA = (defect: Defect): boolean =>
    config.fileAssertionFailures || defect.classification !== 'Assertion Failure';
  const allDefects = ledgerDefects.filter(FILE_TO_BUGZILLA);
  const excludedCount = ledgerDefects.length - allDefects.length;
  if (config.fileAssertionFailures) {
    console.log(
      `${LOG} BUGZILLA_FILE_ASSERTION_FAILURES=true - filing ALL ${allDefects.length} ledger ` +
        `record(s), including test failures that are not API defects.`
  );
  }
  if (excludedCount > 0) {
    console.log(
  ...
  let commented = 0;
  let failed = 0;
  /** Defects whose ticket is already closed as INVALID / WONTFIX / WORKSFORME. */
  let judgedSkipped = 0;
  /** Tickets closed FIXED whose defect came back - reopened rather than re-filed. */
  let reopened = 0;
  ...
  for (const defect of defects) {
    ...
    try {
      ...
      if ('judged' in existing) {
        judgedSkipped += 1;
        console.log(
          `${LOG} ${defect.id} not filed - bug ${existing.judged} is closed as ${existing.resolution}`,
        );
        results.push({ defectId: defect.id, bugzillaId: existing.judged, action: 'skipped-judged' });
        continue;
      }

      let bugId: number;
      let action: 'created' | 'commented' | 'reopened';
      let alias: string | undefined;

      if ('reopen' in existing) {
        const reopenResult = await reopenBug(
          config,
          existing.reopen,
          `Reopening: this defect was observed again in run ${report.generatedAt} ` +
            `(environment ${report.environment}), after the ticket was resolved ${existing.resolution}.` +
            ' The fault is tracked here rather than under a new ticket number.'
        );
        if ('error' in reopenResult) {
          failed += 1;
          console.log(`${LOG} ${defect.id} not reopened on bug ${existing.reopen} - ${reopenResult.error}`);
          results.push({ defectId: defect.id, bugzillaId: existing.reopen, action: 'failed', error: reopenResult.error });
          continue;
        }
        reopened += 1;
        console.log(`${LOG} ${defect.id} reopened bug ${existing.reopen} (was ${existing.resolution})`);
        results.push({ defectId: defect.id, bugzillaId: existing.reopen, action: 'reopened' });
        continue;
      }
      ...
      if ('id' in existing) {
        ...
        const createResult = config.clientAlias
          ? await createBugWithAlias(config, fields, 'KPA')
  ...
  if (reopened > 0) {
    console.log(
      `${LOG} ${reopened} defect(s) reopened an existing ticket instead of filing a new one ` +
        `- they were resolved earlier but have come back.`
    );
  }

  if (judgedSkipped > 0) {
    console.log(
      `${LOG} ${judgedSkipped} defect(s) NOT filed - already closed as INVALID/WONTFIX/WORKSFORME. ` +
        `They remain in BUG_REPORT.md; only the re-filing is suppressed.`
    );
  }
