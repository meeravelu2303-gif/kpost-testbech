...
export interface ReportDefect {
   ...
   * hundred times, which `occurrences` cannot: many of those observations come from tests that
   * passed the rest of their assertions.
   */
  ledgerOccurrences?: number;
  /** Every distinct endpoint the defect was observed at, for the blast-radius column. */
  affectedEndpoints?: Array<{ method: string; endpointPath: string; module: string; occurrences: number }>;
...
/*
 * `environmentName` and `hostOf` used to be defined here. They now live in
 * `src/utils/environment.ts` (imported above) so that `bugTracker` can label BUG_REPORT.json
 * with the same string this report shows — the dashboard ingests that file, and it was
 * displaying a raw URL in its Env column.
 */
