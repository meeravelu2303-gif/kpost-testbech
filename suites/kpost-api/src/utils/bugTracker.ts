...
export function buildCurl(record: {
  ...
  if (record.requestBody !== undefined) {
    ...
    /*
     * `curl -d` defaults to `application/x-www-form-urlencoded`, and so does Postman when it
     * imports a curl carrying a body with no content type. A JSON payload pasted from a ticket
     * therefore arrives as one urlencoded form KEY rather than as a JSON document, and the
     * endpoint answers something the test never saw — on 2026-08-28 that turned a real finding
     * on POST /taWallet/sendCommunicationMessage into an unreproducible 401.
     *
     * The bench sends JSON, so the repro has to say so. Emitted only when the test did not
     * capture a Content-Type of its own, so an assertion that deliberately sends a different
...
export function resolveModule(endpointPath: string): { module: string; team: string } {
   ...
   * all. Specs build `META.path` from the path constant (`/v2/profile/downloadProfileImage`)
 ...
 *
 * It legitimately exceeds the 408 operations in `swagger.json`, and that is not double
...
export function recordBug(input: BugInput): string {
  ...
  const owner = input.owner ?? `Backend Dev - ${resolved.team} Team`;
 ...
 *
 * The ranking follows what KPOST cannot operate without: identity and the payment rail
 * (a breach or an outage here stops the business), then the communication core the product
 * exists to provide, then the shared platform data every module reads, then content and
 * third-party integrations. A module not listed ranks at the boundary between core and
 * platform (50) — visible, not buried, pending a human placing it.
 *
 * This is deliberately *not* the removed `[tierN]` tag: that was a coarse label written onto
 * every ticket and filtered on. This is an invisible sort key. Severity still decides the
 * band; this only decides order within it, so the most business-critical Critical defect —
 * an auth or payment showstopper — sits at the very top of the P0 list.
 */
const COMPONENT_CRITICALITY: Readonly<Record<string, number>> = {
  // identity & security — a breach or lockout here is existential
  'Authentication V2': 0,
  'Authentication - Medium & Large Enterprise': 1,
  'Crypto - Payload Encryption Key': 2,
  'User Profile V2': 3,
  // money — direct financial impact
  'TA Wallet Payments': 5,
  'Integration - TA Wallet Callback': 6,
  'Integration - RazorPay Payments': 7,
  // communication core — the product's reason to exist
  'Katchup Messaging V2': 10,
  'Kall (Voice/Video) V2 - current': 11,
  'Contacts Directory V2': 12,
  'Groups V2': 13,
  // administration & shared platform data every module reads
  'Company Administration': 20,
  'Common Reference Data & Utilities V2': 21,
  'Dashboard V2': 22,
  'General Settings': 23,
  // content, productivity & third-party integrations
  'Kdiary - Schedules, Events & Reports': 60,
  'KWord Documents': 61,
  KPresentation: 62,
  Knews: 63,
  'Integration - RedBus Bus Booking': 64,
  'Integration - AI Assistant': 65,
  'Integration - MetaDee AI': 66,
  'Integration - Voice / Speech-to-Text': 67,
  'Integration - AWS S3 Pre-signed URLs': 68,
  'Integration - E-commerce Catalogue': 69,
  'Firebase Diagnostics': 70,
  'kpost-webservice-application': 71,
};
...
function renderDashboard(records: BugRecord[], meta: RunMeta): string {
  ...
  return `# KPOST API — Automated Bug Report
...
/**
 * Assigns `BUG-V2-<MODULE>-NNN` labels.
 *
 * The `V2` segment records that the suite now covers the **active** API surface only; the
 * deprecated V1 stacks were removed from the run, and their standing risk is tracked in
 * `V1_DECOMMISSION_RISK.md` rather than in this ledger. Without the segment, a ticket filed
 * today and one filed from the archived V1 run would be indistinguishable in a tracker.
 *
...
function assignDisplayIds(records: BugRecord[]): void {
  ...
  for (const record of records) {
    ...
    record.displayId = `BUG-V2-${token}-${String(next).padStart(3, '0')}`;
  }
}
