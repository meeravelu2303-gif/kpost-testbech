...
function collidingInput(overrides: Partial<BugInput> = {}): BugInput {
  return {
    ...
    module: 'Auth',
    method: 'GET',
    endpointPath: '/userDetails/getAllUser',
...
test.describe('bug identity is the defect, not the test that found it', () => {
  ...
  test('an explicit dedupeKey collapses a systemic defect across endpoints too', () => {
    ...
    const first = recordBug(
      collidingInput({ testId: 'test-gamma', dedupeKey: key, endpointPath: '/admin/a' })
    );
    const second = recordBug(
      collidingInput({ testId: 'test-delta', dedupeKey: key, endpointPath: '/admin/b' })
    );
...
test.describe('grouping metadata', () => {
  test('ownership follows the module holding the most affected endpoints', () => {
    ...
    const ids = [
      recordBug(
        collidingInput({ testId: 't1', dedupeKey: key, endpointPath: '/admin/x', module: 'Alpha' })
      ),
      recordBug(
        collidingInput({ testId: 't2', dedupeKey: key, endpointPath: '/admin/y', module: 'Beta' })
      ),
      recordBug(
        collidingInput({ testId: 't3', dedupeKey: key, endpointPath: '/admin/z', module: 'Beta' })
      ),
    ];
  ...
  test('the same fault on two endpoints is one defect under the default fault grouping', () => {
    ...
    const first = recordBug(collidingInput({ testId: 'fg-1', title, endpointPath: '/admin/one' }));
    const second = recordBug(collidingInput({ testId: 'fg-2', title, endpointPath: '/admin/two' }));
