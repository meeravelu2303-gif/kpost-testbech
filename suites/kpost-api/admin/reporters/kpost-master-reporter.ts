...
export default class KpostMasterReporter implements Reporter {
   ...
   *
   * Counted here rather than read from `model.totals.total`, which is derived from the *suite*
   * and therefore reports every test Playwright knows about whether or not it ran. Under
   * `npx playwright test --list` the suite is fully populated while nothing executes, so a gate
   * trusting that total sees the whole suite as "executed" and cheerfully publishes a listing
   * to the QA Dashboard. `onTestEnd` fires once per test that reached a verdict, so it cannot
  ...
  async onEnd(result: FullResult): Promise<{ status?: FullResult['status'] } | void> {
    try {
       ...
       * and long before the publishers further down `playwright.config.ts` read it. A run
       * that never happened must not reach the QA Dashboard or Bugzilla, and those reporters
       * consult this file rather than re-deriving the judgement themselves.
       */
