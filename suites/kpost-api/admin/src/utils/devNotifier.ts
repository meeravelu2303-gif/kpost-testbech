...
/*
 * Ordered most severe first. The pre-rename words are kept alongside the current ones so a
 * digest built from an archived `BUG_REPORT.json` still sorts correctly instead of dropping
 * its bands to the end of the list. See `bugTracker.normalizeSeverity`.
 */
const SEVERITY_ORDER = ['Critical', 'Major', 'Minor', 'Medium', 'Trivial', 'Low'];
