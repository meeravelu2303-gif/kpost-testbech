...
export default class BugReporter implements Reporter {
  ...
  async onEnd(_result: FullResult): Promise<void> {
     ...
     * test then reports `outcome() === 'skipped'` — which compiles to a BUG_REPORT.json of
     * "N tests, 0 defects" that replaces a real report. Listing the suite would silently
     * destroy the evidence from the last real run, and the digest would then announce the
     * result as CLEAN.
     *
